...
md"""
Section 1 We have a class midpoint survey!! 
...
# ╔═╡ 049b86a7-e763-4bfa-9de7-975a2fe8dee7
md"""
# 1. Class Midpoint Survey
"""

# ╔═╡ 5013d6a3-2ba6-47d8-85b8-ac1b8f12842b
md"""
Now that we are a bit over half way through the semester, we want to check in with everyone and hear both what we are doing well and how we can improve for the remainder of the semester.

The first part of the homework is for completing the survey. Since it is completely anonymous, we provide a passphrase at the end of the survey. 

[Julia Computation Class Midpoint Survey](https://forms.gle/xTKs1qyaPyMvY4QS8)
"""

# ╔═╡ faa2a1cd-9686-4e37-bee9-f610cfb582f9
passphrase = "WonderfulJulia"

# ╔═╡ fb6e4a89-7a07-4032-9bff-54599050bbdc
begin
	function check_passphrase(input_passphrase)
	    encoded_pass = base64encode(input_passphrase)
	    checks = [
	        ()-> begin return length(encoded_pass) == 20 end,
	        ()-> begin return encoded_pass[1] == 'V' end,
	        ()-> begin return sum([Int(c) for c in encoded_pass]) % 5 == 0 end,
	        ()-> begin return encoded_pass == "V29uZGVyZnVsSnVsaWE=" end, 
	        ()-> begin return contains(encoded_pass, "Julia") end
	    ]
	    for check in checks
	        if check == checks[4] && !check()
	            return almost(md"**Please complete the survey**")
	        end
	    end
	    return correct(md"**Survey Complete**")
	end
...
begin
	struct TemperatureSeriesConstant{Y<:Number,V<:Number}
		years::Vector{Y}
		values::Vector{V}
	end
	
	function (temp::TemperatureSeriesConstant)(t::Number)
		k = searchsortedfirst(temp.years, t)
		return temp.values[k]
	end
end
...
let
	...
	@code_warntype temp_constant(mean(temp_years))
end
...
begin
	struct TemperatureSeriesLinear{Y<:Number,V<:Number}
		years::Vector{Y}
		values::Vector{V}
	end
	
	function (temp::TemperatureSeriesLinear)(t::Number)
		k = searchsortedfirst(temp.years, t)
		y1, y2 = temp.years[k-1], temp.years[k]
		T1, T2 = temp.values[k-1], temp.values[k]
		a1, a2 = t - y1, y2 - t
		return a1 * T1 + a2 * T2
	end
end
...
let
	...
	@code_warntype temp_linear(mean(temp_years))
end
...
function dynamics2(T, p, t)
	return inv(heat_capacity) * (feedback * (T_equilibrium - T) + greenhouse_effect(t))
end

# ╔═╡ 26cf28b4-6fe9-4560-be5b-cb8fd2bba414
problem2 = ODEProblem(dynamics2, T_preindustrial, (t0, tf))
...
begin
	century_average = mean(solution2.u[1900 .<= solution2.t .< 2000])
	pl = plot(xlabel="Year", ylabel="Temperature anomaly", title="Climate model fitting")
	plot!(pl, solution2.t, solution2.u .- century_average, label="model")
	plot!(pl, temp_years, temp_values, label="measurements")
end
...
md"""
> Because some areas of the temporal axis may require more precision than others.
"""
...
# ╔═╡ 03c4860f-a5a8-46da-827e-a84e9f25aabc
md"""
> The solution of an `ODEProblem` is a callable struct and it is interpolating between the individual points.
"""
...
md"""
- Heat capacity is inversely related to growth rate as increasing the heat capacity will slow the rate at which the temperature changes as a result of a change in energy. 
- Feed back is inversely related to the growth rate as it changes the measured effect of a temperature gradient without effecting the greenhouse gas effect. 
- Forcing is directly related to the growth rate as it influences the factor of the greenhouse effect. 
"""
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "b79e5c5270344f9a2d33f7c50440c063d29e64d9"
