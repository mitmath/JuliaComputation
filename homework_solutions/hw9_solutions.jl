...
let url = "https://github.com/mitmath/JuliaComputation/raw/Fall22/homeworks/images/lineland.png"
	img = download(url)
	load(img)
end

# ╔═╡ f205fe06-9849-435f-963c-d7596592c979
function meeting_matrix(p::Vector{R}) where {R}
	n = length(p)
	M = Matrix{R}(undef, n, n)
	for i in 1:n 
		M[i, i] = zero(R)
	end
	for i in 1:n, j in (i+1):n
		M[i, j] = p[i] * prod(one(R) - p[k] for k in (i+1):(j-1); init=one(R)) * p[j]
	end
...
if @isdefined meeting_matrix
	let
		p = collect(1:5) ./ 5
		M = Symmetric(meeting_matrix(p))
	end
end

# ╔═╡ 2bf674a4-d039-48af-8642-1160873b2f70
struct MeetingMatrix{R} <: AbstractMatrix{R}
	p::Vector{R}
end

# ╔═╡ 02f7ec49-1ab9-4ae0-a3ee-baee0dd25ea6
function Base.size(M::MeetingMatrix)
	n = length(M.p)
	return (n, n)
end

# ╔═╡ 2d3f2591-3d7b-41b6-9129-777168379510
function Base.getindex(M::MeetingMatrix{R}, i, j) where {R}
	p = M.p
	if i == j
		return zero(R)
	elseif i < j
		return p[i] * prod(one(R) - p[k] for k in (i+1):(j-1); init=one(R)) * p[j]
	else
		return getindex(M, j, i)
	end
end
...
if (@isdefined meeting_matrix) && (@isdefined MeetingMatrix)
	let
		p = collect(1:4) ./ 5
		M1 = meeting_matrix(p)
		M2 = MeetingMatrix(p)
		Base.summarysize(M1), Base.summarysize(M2)
	end
end

# ╔═╡ 8eaaee9d-ac73-4d0e-933d-7070845d74fb
if (@isdefined meeting_matrix) && (@isdefined MeetingMatrix)
	let
		p = collect(1:4) ./ 5
		M1 = meeting_matrix(p)
...
function multiply_row_neighbors_matrix(p::Vector{R}, i, x) where {R}
	n = length(p)
	y = zero(R) * x[i]
...
function multiply_meeting_matrix(p::Vector, x)
	D = Diagonal(p)
	return D * multiply_neighbors_matrix(p, D * x)
end
...
md"""
Note that the Pluto (and REPL) display will compute a few coefficients of the `MeetingMatrix`, but these coefficients are not actually stored within it. 
"""
...
md"""
We sacrificed computation time for memory, because every time we use the `MeetingMatrix` we need to recompute coefficients from scratch.
"""
...
md"""
In some situations, it might be even more efficient to consider matrices as linear operators, i.e. functions that apply to vectors.
We have seen an example in HW2, and now we give another one.
"""
...
md"""
!!! danger "Task 1.2.7"
	Give another expression for $M(p)$ of the form $D(p) N(p) D(p)$, where $D(p)$ is a diagonal matrix whose coefficients only involve $p$, and $N(p)$ is the neighbors matrix whose coefficients only involve $\bar{p}$.
"""

# ╔═╡ 3f6001f1-3bf3-4a89-92ca-07a8ee0bab75
md"""
We have $M(p) = D(p)N(p)D(p)$ where
```math
N_{ij}(p) = \begin{cases} 
0 & \text{if $i = j$} \\
\prod_{k=i+1}^{j-1} (1-p_k) & \text{otherwise}
\end{cases}
\qquad
\text{and}
\qquad
...
\end{cases}
```
which for $n = 5$ yields
```math
N(p) =
\begin{pmatrix}
0 & 1 & \bar{p}_2 & \bar{p}_2 \bar{p}_3 & \bar{p}_2 \bar{p}_3 \bar{p}_4 \\
1 & 0 & 1 & \bar{p}_3 & \bar{p}_3 \bar{p}_4 \\
\bar{p}_2 & 1 & 0 & 1 & \bar{p}_4 \\
\bar{p}_3 \bar{p}_2 & \bar{p}_3 & 1 & 0 & 1 \\
\bar{p}_4 \bar{p}_3 \bar{p}_2 & \bar{p}_4 \bar{p}_3 & \bar{p}_4 & 1 & 0
\end{pmatrix}
```
"""
...
# ╔═╡ e548c0f3-bab0-4c1f-a282-6d4b30389b9f
md"""
Interestingly, the `\` operator adapts to the type of matrix we give it.
"""
...
md"""
Multiple dispatch chooses the best method based on the type of the matrix $A$, because efficient implementations exist for specific structures.
"""
...
# ╔═╡ 7388a128-6680-4527-8ed4-e2fc09d7a33a
md"""
!!! danger "Task 1.3.2"
	Can multiple dispatch choose a linear solve algorithm based on the size of the matrix $A$?
"""

# ╔═╡ 2dc9cf03-33ab-44ab-a100-a865f20a4098
md"""
No, since multiple dispatch is based on the type information that can be known at compile-time, not values, which are generally only known at runtime.
If the size were stored in the type information, then it would be able to.
"""
...
let url = "https://github.com/mitmath/JuliaComputation/raw/Fall22/homeworks/images/atmosphere.png"
	img = download(url)
	load(img)
end
...
```math
\text{absorbed longwave} - \text{emitted longwave} + \text{absorbed shortwave} = 0
```
...
md"""
We have $A = M(\varepsilon) - E$ where $E = \mathrm{diag}(\varepsilon_1, 2\varepsilon_2, \dots, 2\varepsilon_n)$.
"""
...
function longwave_matrix(ε::Vector)
	M = meeting_matrix(ε)
	E = Diagonal(vcat(ε[1], 2 .* ε[2:end]))
	return Symmetric(M .- E)
end
...
function LongwaveMatrix(ε::Vector)
	M = MeetingMatrix(ε)
	neg_E = Diagonal(vcat(-ε[1], -2 .* ε[2:end]))
	return LazyMatrixSum(M, neg_E)
end
...
function multiply_longwave_matrix(ε::Vector, x)
	y = multiply_meeting_matrix(ε, x)
	y[1] -= ε[1] * x[1]
	@views y[2:end] .-= 2 .* ε[2:end] .* x[2:end]
	return y
end
...
if @isdefined multiply_longwave_matrix
	let
		A = x -> multiply_longwave_matrix(ε_val, x)
		b = shortwave_vector(β_val, S₀_val, σ_val)
		x, _ = linsolve(A, b)
		T = x .^ (1/4)
		fahrenheit.(T)
...
md"""
The upper layer cools down very slightly.
"""
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "a886e2b546f5a84f97e4b234eb0691a3ae2ede63"
