...
function shrink_vertical_n(img, n)
	carved, mark_carved = shrink_n(rotl90(img), n)
	return (rotr90.(carved), rotr90.(mark_carved))
end
...
function shrink_to_size(img, dims)
	...
	while any(size(img) .> dims)
		# Find the minimal energy of removing a vertical path
		least_E_col_rm, dirs_col_rm = least_edgy(e)
		min_E_col_rm, min_j = findmin(@view least_E_col_rm[1, :])
		avg_E_col_rm = 	if size(img, 2) > cols 
							min_E_col_rm / size(img, 1)
						else 
							Inf # apply infinite cost to removing
								# too many columns
						end
		
		# Find the minimal energy of removing a horizontal path
		least_E_row_rm, dirs_row_rm = least_edgy(rotl90(e))
		min_E_row_rm, min_i = findmin(@view least_E_row_rm[1, :])
		avg_E_row_rm = 	if size(img, 1) > rows 
							min_E_row_rm / size(img, 2)
						else 
							Inf # apply infinite cost to removing
								# too many rows
						end
		
		# Remove the seam with the lower average energy
		if  avg_E_col_rm < avg_E_row_rm
			seam = get_seam_at(dirs_col_rm, min_j)
			img = rm_path(img, seam)
			e = rm_path(e, seam)
			marked_img = mark_path(img, seam)
		else
			seam = get_seam_at(dirs_row_rm, min_i)
			img = rotr90(rm_path(rotl90(img), seam))
			e = rotr90(rm_path(rotl90(e), seam))
			marked_img = rotr90(mark_path(rotl90(img), seam))
		end

		push!(imgs, img)
 		push!(marked_imgs, marked_img)
	end
