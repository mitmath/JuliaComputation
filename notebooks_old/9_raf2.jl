...
Let us begin by reviewing how gases absorb radiation. This requires a review of quantum theory, a field that may not be the forte of some climate skeptics. Any given molecule of a gas can be at different energy levels, depending on the levels occupied by its electrons, but also on the vibration and rotations modes of the molecular bonds. Radiation arrives to a molecule in the form of a photon of energy $h/\lambda$ where $h$ is the Plank constant and $\lambda$ the photon wavelength. The photon can be absorbed by the molecule only if its energy (and hence its wavelength) matches the amount of energy needed to cause the molecule to transition between its energy level and a higher one. Moving electrons to higher orbitals typically requires very energetic photons in the ultraviolet range (shortwaves). Exciting vibrations and rotations requires photons with weaker energies. In particular vibrations where the dipole moment of the molecule changes are excited by photons in the thermal energy range (longwaves) and hence play a crucial role in the Earth's greenhouse effect.
...
As the concentration of CO$_2$ increases so does the number of collision and the broadening of the absorption lines. And voila', the absorptivity/emissivity of the atmosphere increases in the CO$_2$ absorption wavelengths range.
"""
...
begin
	year = 365 * 86400
	C_u = 4000 * 1000 * 100 * 0.7 / year # Heat capacity of ocean mixed layer [Wyr/m^2/°K]
	C_d = 4000 * 1000 * 3900 * 0.7/ year # Heat capacity of deep ocean [Wyr/m^2/°K]
	δT₀ = 0 # Initial temperature perturbation
...
On timescales much longer than τᵗʳ, the upper ocean can be assumed to be in equilibrium and the system of equations reduces to:
...
which can be combined into a single equation for $\delta T_d$:
...
md"""
Let's make a fit to the CO₂ data from Mauna Loa to extrapolate back in time and cover the same temporal range as the temperature data
"""
