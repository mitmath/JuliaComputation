...
# ╔═╡ c6d69c19-92a6-466c-81a2-6a2c78d255ad
using Plots, LinearAlgebra, Triangulate 

# ╔═╡ 2030f9ac-3e09-11ed-35be-eb75d09dea91
md"""
...
# ╔═╡ 878d69c6-7b10-48ca-b726-50224ad2086d
md"""
# Convex Hull
"""

# ╔═╡ e6284251-8f38-4fd5-bb21-13ee20776c07
md"""
...
# ╔═╡ 666bfda8-283b-4756-aedb-4066c2074646
ten_nodes = [ randn(2) for i = 1:10 ] # 10 random points - note array-of-array

# ╔═╡ 9a0eb5bc-9b26-45ad-99b6-8689f4345543
# Plot the points as dots
# The functions first and last simply collects the corresponding x,y indices
scatter(first.(ten_nodes), last.(ten_nodes),
        aspect_ratio=:equal)

# ╔═╡ 574c5144-a65b-45aa-b587-5e81a7ffc2d9
md"""
## Convex hull
...
![Jarvis_march_convex_hull](https://upload.wikimedia.org/wikipedia/commons/thumb/d/de/Jarvis_march_convex_hull_algorithm_diagram.svg/800px-Jarvis_march_convex_hull_algorithm_diagram.svg.png)
(from <https://en.wikipedia.org/wiki/Gift_wrapping_algorithm>)
"""

# ╔═╡ df10c580-30b2-4a23-8bd6-75e658da2b9e
md"""
...
# ╔═╡ 3f15bc42-4bc1-4502-a11d-a40d9f81e0b9
...
# ╔═╡ a90cc361-046a-4e0f-bfd5-64827df550a9
clockwise_oriented([0,0], [1,1], [2,3]) # Should be true

# ╔═╡ 0e6004e8-66c4-4d65-8c07-f34d0d442281
clockwise_oriented([0,0], [1,1], [3,2]) # Should be false

# ╔═╡ 4178854c-6d10-4424-9e10-d036afc2ea43
md"""
...
# ╔═╡ fe0930f5-04e5-40da-9ab5-fafed633ce1a
function convex_hull(p)
    ...
    while length(hull) ≤ 1 || hull[1] != hull[end] # Loop until closed polygon
        # First candidate, any point except current
		nextPoint = hull[end] % length(p) + 1 
        for j = 1:length(p) # Consider all other points
			# If "more to the left", switch
...
begin
	...
	local nodes = [ randn(2) for i = 1:100 ]
	hull = convex_hull(nodes)
	scatter(first.(nodes), last.(nodes),
		    aspect_ratio=:equal)
	plot!(first.(nodes[hull]), last.(nodes[hull]))
end

# ╔═╡ 0008135f-350f-4cd3-9865-6f23c1880ac0
md"""
...
# ╔═╡ 20b0af3a-9447-4b3c-82fa-26b69c995d0f
md"""
...
# ╔═╡ 6ffbfc8c-fe6f-49e9-b1a2-aace751c5c62
...
# ╔═╡ bc45e8a9-81dd-4983-aac9-6aed2ba3db23
begin
	...
	# Plot all lines
	h = plot(aspect_ratio=:equal, legend=:none)
	for line in lines
	...
	for i = 1:n
	    for j = i+1:n
	        isect, pq = linesegment_intersect(lines[i][1], lines[i][2],
				                              lines[j][1], lines[j][2], 0)
	        if isect
	            scatter!([pq[1]], [pq[2]])
	        end
	    end
...
# ╔═╡ 99a20616-d186-4723-b38a-a695e584bf7b
md"""
# Triangulations
...
First let us define a function to plot a triangulation. The function below is based on Plots, and it takes an array of points `nodes` and an array of triangle indices `triangles`.
"""

# ╔═╡ fa5738a1-2a62-4d0b-b67d-71e13b3b72cf
function tplot(nodes,triangles)
    ...
    h = plot(first.(allpv), last.(allpv),
             seriestype=:shape, aspect_ratio=:equal,
             leg=false, fillcolor=RGB(0.8,1,0.8))
end

# ╔═╡ 24c4f210-9eca-4315-b85c-0e7645184500
md"""
We can test the function using a simple set of points and a triangulation:
"""

# ╔═╡ 5c0efb32-e365-49eb-aee5-e18491304d00
begin
	nodes = [[0,0], [1,0], [0,1], [1,2], [2,1]]
	triangles = [[1,2,5], [1,5,3], [3,5,4]]
	tplot(nodes,triangles)
end

# ╔═╡ c1738f8a-a544-49ee-85d1-51f8bd3263a2
md"""
...
Many algorithms have been developed for computing a Delaunay triangulation from a set of points, and here we will simply use an existing implementation from the Matplotlib python library:
"""

# ╔═╡ 6601528a-e058-4ba3-b9d9-6c4d7f24caa1
function delaunay(nodes)
	triin = Triangulate.TriangulateIO()
	triin.pointlist = hcat(nodes...)
	(triout, vorout)=triangulate("Q", triin)
	triangles = [tri for tri in eachcol(triout.trianglelist)]
end

# ╔═╡ 36530462-fc87-49a3-a4b2-c777f1cf41df
md"""
First, let us compute and plot the Deluanay triangulation of the same points as before:
"""

# ╔═╡ 0aafbbda-99b0-4341-a4b8-f1509fbfb708
triangles_delaunay = delaunay(nodes)

# ╔═╡ aa2966bc-f663-4f20-b325-9052d8e6513e
tplot(nodes, triangles_delaunay)

# ╔═╡ c500b176-c2d5-4c10-826a-4312a5aaf81c
md"""
Note that this triangulation is different than our previous one - it satisfies the properties mentioned above.
"""

# ╔═╡ 1397dff9-c109-43f9-a71d-8547e5ec526e
md"""
Next let us compute and plot the delaunay triangulation of a random set of points:
"""

# ╔═╡ 33e23c8e-8320-4833-a0d1-6bfc6c8d2ac5
begin
	local nodes = [ randn(2) for i = 1:50 ]
	local triangles = delaunay(nodes)
	tplot(nodes, triangles)
end
