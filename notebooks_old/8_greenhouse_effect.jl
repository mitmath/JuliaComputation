...
md"""
## Computational thinking and climate modeling
"""
...
md"""
---
#### 1) Start with constant forcing
\
``T' = a``
$(html"<br>") Solution: ``T(t) = at + T_0``
...
#### 2) Add linear damping in temperature
\
``T' = a - bT``$(html"<br>") Solution: ``T(t) =(\frac{a}{b})(1-e^{-bt}) + T_0 e^{-bt}``

Comment:  Equilibrium obtained  (if $b\ne0$)  by solving ``T'=0`` for T giving equilibrium $T=a/b$. Also
...
md"""
#### 3) Add time dependent forcing ``f(t)``
\
...
f(T,(a, b, forcing(c)),t) = a - b*T + forcing(c)(t)
...
T₀ = $(@bind T₀ Slider(-5:.1:15, show_value=true, default=2.0) )

c = $(@bind c Slider(0:.1:5, show_value=true, default=0.0) )
"""

# ╔═╡ 578a80ad-fcf1-4d5c-81e5-205113bbf78d
sol = solve(  ODEProblem( f, T₀, (0, 10.0), (a, b, forcing(c)) ) );

# ╔═╡ bda41881-75c7-4732-9a66-d7947607b1b6
begin
	p = plot( sol , legend=false, background_color_inside=:black , ylims=(-5, 10), lw=3, c=:red)
	# plot direction field:
	    ...
	    ylabel!("T")
	    annotate!(-.5,T₀,text("T₀",color=:red))
	    title!("Solution to T'(t) = a - bT + forcing(c)(t)")
end

# ╔═╡ 2ca14de2-2f6f-4e0c-a324-ad850c9132e5
md"""
---
---
"""

# ╔═╡ e31693a1-70e1-48f2-93ba-c2ec13ea72ee
md"""
## Earth energy budget
...
Both the heat capacity per unit area (which depends on the Earth's composition) and the area of the earth do not change in time. We can thus write:

$$\text{\color{brown}{change in heat content}}
\color{brown}{= 4 \pi R^2 C \frac{d T}{dt}}$$
"""

# ╔═╡ 95085dc9-fd7b-4e48-a75c-0167137ddb64
C = 355.; # atmosphere and upper-ocean heat capacity [Wyr/m^2/°K]

# ╔═╡ 35225212-6110-4e7b-9abc-ad0bac208dd0
md"""
---
...
html"""<img src="https://chem.libretexts.org/@api/deki/files/344337/1.1.2.svg?revision=2&size=bestfit&width=746&"height=319>"""
...
html"""<img src="https://cdn.britannica.com/10/7710-050-C2D756A7.jpg"height=319>"""
...
# ╔═╡ bee4395c-3e55-443e-831f-c50d6b88c1a3
begin
	clight = 299792458
	h = 6.63e-34
	κ = 1.38e-23
	BlackBody(Tb) = λ -> (2*h*clight^2)/λ^5/(exp(h*clight/λ/κ/Tb)-1)
end

# ╔═╡ c2b34a2d-7271-43a1-a60d-9f0a0bee6f6b
md"""
Tb = $(@bind Tb Slider(200:100:1000, show_value=true, default=200) )
"""

# ╔═╡ 46ef400d-6001-4c6f-9925-49ab751a51e6
begin
	plot( BlackBody(Tb), 5e-7, 5e-5 , legend=false, background_color_inside=:black , ylims=(1e5, 1e10), lw=3, c=:red, yaxis=:log)
	    ylabel!("Power [W/m²/sr/m]")
	    xlabel!("λ [m]")
	    title!("Power emitted per unit area by a blackbody")
end
...
# ╔═╡ 1f2ab21d-bd7f-4dab-8cd1-33b96d85ed08
σ = 5.670374419*10^(−8) # [W/m^2/K^4]

# ╔═╡ 3a355c17-770d-4ce4-a8bf-117ce6616788
md"""The Sun has a mean surface temperature of ~6000K and emits radiation in the visible range $0.4 μm < \lambda < 0.7 μm$. The Earth has a mean surface temperature of 300K and emits radiation in the infrared range: $4 μm < \lambda < 40 μm$."""

# ╔═╡ 8c930e06-8f58-49be-9724-5536d3156067
html"""<img src="https://learningweather.psu.edu/sites/default/files/Lesson3/plank0303.jpg" width=625>"""

# ╔═╡ bddce5ec-8f5c-4025-96fb-1443d4e347aa
md"""
#### 3) Incoming solar radiation (shortwave)

At Earth's orbital distance from the Sun, the power of the Sun's rays that intercept the Earth per unit area is equal to
"""

# ╔═╡ add2a5e8-4a28-4395-9d61-2077fd31e0f4
S = 1366; # solar insolation [W/m^2]  (energy per unit time per unit area)

# ╔═╡ b517c441-b068-4778-9af0-6ce82c5bbfc7
html"""<img src="https://cdn.britannica.com/38/106538-050-D9A57E9E/trend-reconstruction-changes-brightness-stars-Lean-Sun-1900.jpg" width=625>"""

# ╔═╡ ca5b41d2-b511-486a-91fe-cceb8f7282c3
md"A small fraction"

# ╔═╡ 875e146c-d72f-46b4-9d82-21110e81ca9b
α = 0.3; # albedo, or planetary reflectivity [unitless]

# ╔═╡ f1df983a-9d70-48c8-a7ee-7ba3bd9c0fa5
md"""
...
Since the incoming solar rays are all approximately parallel this far from the Sun, the cross-sectional area of the Earth that intercepts them is just a disc of area $\pi R^{2}$.

![](https://www.e-education.psu.edu/earth103/sites/www.e-education.psu.edu.earth103/files/module03/fig6sol.png)

The total solar radiation absorbed by the Earth per unit time is thus:

$\textcolor{black}{\text{absorbed solar radiation} \equiv S(1-\alpha)\pi R^2}$
"""
...
$$\color{brown}{C \frac{d T}{dt}} \color{black}= \color{red} \frac{(1-\alpha)S_0}{4}$$

"""

# ╔═╡ 717a83a5-70d2-453b-b7da-f05c366545fd
md"""

The solar energy absorbed per unit time and unit area by the Earth is therefore:

$$\color{red}\text{solar energy absorbed} = \frac{(1-\alpha) S_0}{4}$$
...
# ╔═╡ 7187ae25-239d-4752-898e-6674009b5de6
 p1 = ODEProblem( (temp, p, t)-> (1/C) * absorbed_solar_radiation, temp₀,  (0, timespan) )

# ╔═╡ fb61866f-c3f9-40d2-848b-f7c54ad6d447
begin
	solution1=solve(p1)
	solution1.u.-=273.15
	plot(solution1,       legend = false, 
...
# ╔═╡ 03ccad3d-09ed-4621-9845-ceb605072ebf
md"""
The Earth's temperature would reach 400°C, above boiling point of water, after 1000 years.
"""

# ╔═╡ 2b66ffaf-a9a3-4ca2-a6e9-732912c9d48e
md"""
---
#### 4) Earth's energy budget without an atmosphere
"""
...
# ╔═╡ b12209a8-2f34-4daa-8288-d91339a940f9
 p2 = ODEProblem( (temp, p, t)-> (1/C) * (absorbed_solar_radiation-σ*temp^4), temp₀,  (0, timespan) )

# ╔═╡ 5eb54ebb-0ebd-47a7-b5c0-a3059f82aa9e
begin
	solution2=solve(p2)
	solution2.u.-=273.15
	plot(solution2,        legend = false, 
		 background_color_inside = :black,
		                  xlabel = "years",
	                      ylabel = "Temperature °C",
                          ylim   = (-273.15,100))
	hline!( [temp₀,temp₀] ,c=:white,ls=:dash)
	annotate!( 700, 25+temp₀-273.15, text("Initial Temperature = $(temp₀) K",color=:white))
	title!("Earth's heat budget without an atmosphere")
end
...
$$\color{blue}{T} ={\sqrt[4]{\frac{(1-\alpha)S_0}{4 \sigma}} = 255K = -18^{\circ}C}$$

This is clearly too cold, because the Earth's is not solid frozen as we know. What's the problem with our calculation?
...
md"""
---
#### 5) Earth's energy budget with a thick atmosphere
"""
...
&\color{brown}{C_o \frac{d T_o}{dt}} \color{black}{=} \color{red}{\frac{(1-\alpha)S_0}{4}} \color{black}{\,+\,}  \color{blue}{\sigma T_a^4} \color{black}{\,-\,}\color{blue}{\sigma \,T_o^4}
\end{align}
...
function ghgmodel!(du, u, p, t)

	T_a = u[1]
	T_o = u[2]
	
    du[1] = (1/C_a) * (σ*T_o^4-2*σ*T_a^4)
    du[2] = (1/C_o) * (absorbed_solar_radiation+σ*T_a^4-σ*T_o^4)

end

# ╔═╡ 3aa13ecd-9d4a-4b41-b15f-f85e2b8f8bf6
begin
	p3 = ODEProblem(ghgmodel!, [Ta₀; To₀], (0.,timespan))
end

# ╔═╡ 6613b68f-9997-4230-b757-a5ac46e908ad
begin
	solutioneq3=solve(p3)
	for i in eachindex(solutioneq3.u)
	solutioneq3.u[i] .-= [273.15, 273.15]
	end
	plot(solutioneq3,       label = ["Tₐ" "Tₒ" "Tₒ"], 
		 background_color_inside = :black,
		                  linecolor = [:red :blue],
		                  xlabel = "years",
	                      ylabel = "Temperature °C",
                          ylim   = (-273.15,100))
	hline!( [temp₀,temp₀] ,c=:white,ls=:dash,label=false)
	annotate!( 700, 25+temp₀-273.15, text("Initial Temperature = $(temp₀) K",color=:white))
	title!("Earth's heat budget with a thick atmosphere")
end
...
# ╔═╡ d536ffda-117a-4434-9191-8a2c12df825e
md"""
...
```math
\begin{align}
&\color{brown}{0} \color{black}{=} \color{blue}{\sigma \,T_o^4} \color{black}{-} \color{blue}{2 \sigma \bar T_a^4}\\
&\color{brown}{C_o \frac{d T_o}{dt}} \color{black}{=} \color{red}{\frac{(1-\alpha)S_0}{4}} \color{black}{\,+\,}  \color{blue}{\sigma \bar T_a^4} \color{black}{\,-\,}\color{blue}{\sigma \,T_o^4}
\end{align}
```
...
```math
\begin{align}
\color{brown}{C_o \frac{d T_o}{dt}} \color{black}{=} \color{red}{\frac{(1-\alpha)S_0}{4}} \color{black} \color{black}{\,-\,}\color{blue}{\frac{1}{2}\sigma \,T_o^4}
\end{align}
```

	"""

# ╔═╡ de60ddf6-b77d-4960-b3d1-7018abf3b178
function ghgeqmodel!(du, u, p, t)
    ...
    du[1] = (1/C_a) * (σ*T_o^4-2*σ*T_a^4)
    du[2] = (1/C_o/2^(1/4)) * (absorbed_solar_radiation-0.5*σ*Teq_o^4)
    du[3] = (1/C_o) * (absorbed_solar_radiation+σ*T_a^4-σ*T_o^4)
	du[4] = (1/C_o) * (absorbed_solar_radiation-0.5*σ*Teq_o^4)

end

# ╔═╡ 8da81688-e1d7-4d8d-8e2d-76ac7b17cdbe
begin
	peq3 = ODEProblem(ghgeqmodel!, [Ta₀; To₀; Ta₀; To₀], (0.,timespan))
end

# ╔═╡ 20915df5-5385-4b82-b3e5-8637171f0b0b
begin
	solution3=solve(peq3)
	for i in eachindex(solution3.u)
	solution3.u[i] .-= [273.15, 273.15, 273.15, 273.15]
	end
	plot(solution3,        label = ["Tₐ" "̅̅Tₐ: atm in equilibrium" "Tₒ" "Tₒ: atm in equilibrium"],
		                   legend = :topleft,
		 background_color_inside = :black,
...
# ╔═╡ 4733e6c8-acb3-4ed8-8aa5-ee0cbec1a484
md"""
---
...
```math
\begin{align}
&\color{brown}{C_a \frac{d T_a}{dt}} \color{black}{=} \color{blue}{\epsilon\sigma \,T^4} \color{black}{-} \color{blue}{2 \epsilon\sigma T_a^4}\\
&\color{brown}{C_o \frac{d T_o}{dt}} \color{black}{=} \color{red}{\frac{(1-\alpha)S_0}{4}} \color{black}{\,+\,} \color{blue}{\epsilon\sigma \,T_a^4} \color{black}{\,-\,} \color{blue}{\sigma \,T_o^4}
\end{align}
```
...
```math
\begin{align}
&\color{brown}{C_o \frac{d T_o}{dt}} \color{black}{=} \color{red}{\frac{(1-\alpha)S_0}{4}} \color{black}{\,-\,} \color{blue}{\frac{2-\epsilon}{2}\sigma \,T_o^4}
\end{align}
```
...
epsilon = 0.75; # Earth's emissivty nin preindustrial climate

# ╔═╡ af69dae3-48c4-4327-a0e4-d29734b3c7e1
pleaky3 = ODEProblem( (temp, p, t)-> (1/C) * (absorbed_solar_radiation - (2-epsilon)/2*σ*temp^4), temp₀,  (0, timespan) )

# ╔═╡ 185321ec-c7b3-4deb-883c-fdac3e3193ff
begin
	solutionleaky3=solve(pleaky3)
	solutionleaky3.u .-= 273.15
	plot(solutionleaky3,   label = ["Tₐ"], 
		 background_color_inside = :black,
		                  xlabel = "years",
	                      ylabel = "Temperature °C",
                          ylim   = (-273.15,100))
	hline!( [temp₀,temp₀] ,c=:white,ls=:dash,label=false)
	annotate!( 700, 25+temp₀-273.15, text("Initial Temperature = $(temp₀) K",color=:white))
	title!("Earth's heat budget with a leaky atmosphere")
end

# ╔═╡ 1b692c88-d7e2-4d37-a81f-3b5cbfaf187e
md"""

The equilibrium temperature of the ocean is now a much more reasonable

$$\color{blue}{T_o} ={\sqrt[4]{\frac{2}{2-\epsilon}\frac{(1-\alpha)S_0}{4 \sigma}} = 287K = 13^{\circ}C}$$
