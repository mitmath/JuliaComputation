...
md"""
In this notebook, we explain the fundamental principle behind [Graphs.jl](https://github.com/JuliaGraphs/Graphs.jl), the reference library for graphs in Julia.
...
# ╔═╡ 37625502-8d06-43ef-b6cc-be76459d7946
md"""
...
md"""
## Transposed dense adjacency matrix
"""
...
begin
	...
	function SparseMatrixGraph(n, edges)
		us, vs, ws = Int[], Int[], Float64[]
		for (u, v, w) in edges
			push!(us, u); push!(vs, v); push!(ws, w)
		end
		A = sparse(us, vs, ws, n, n)
...
md"""
## Transposed sparse adjacency matrix
"""
...
function outneighbors(g::TransposedSparseMatrixGraph, u)
	Aᵀ = g.Aᵀ
	u_col_min = Aᵀ.colptr[u]
	u_col_max = Aᵀ.colptr[u+1] - 1
    return @view Aᵀ.rowval[u_col_min:u_col_max]
end
...
Return the length of a shortest path from `s` to all other vertices in the weighted graph `g`.
"""
...
md"""
# 3. Heuristics for hard problems
"""

# ╔═╡ c065c3b1-33da-40a4-99cf-fc4fe19b82ff
md"""
## TSP definition
"""

# ╔═╡ 51fa53d7-b341-48f5-83e6-769b3ffec0fb
md"""
The Traveling Salesperson Problem requires finding the shortest cycle that goes through each vertex of a graph once (a Hamiltonian cycle).
It has many [practical](https://www.math.uwaterloo.ca/tsp/star/index.html) [applications](https://www.math.uwaterloo.ca/tsp/uk/index.html) and even a dedicated [website](https://www.math.uwaterloo.ca/tsp/).

Visualization: [here](https://visualgo.net/en/tsp)

It is an [NP-hard](https://en.wikipedia.org/wiki/Computational_complexity_theory) optimization problem, which means a polynomial solution algorithm is unlikely to exist (unless we count [this one](https://xkcd.com/399/)).
So how can we find a good approximate solution quickly?
"""

# ╔═╡ a7ff54f9-98b2-4ff3-96ae-9ed9a5f74363
md"""
## Initial solution
"""

# ╔═╡ 253ed27c-bd36-45d8-b86e-767ebc4c34b9
md"""
The first step is to construct a feasible solution, even if it is bad.
Examples of initializations:

- Random
- Greedy heuristic (e.g. nearest-neighbor)
- Approximation algorithm (e.g. [Christofides](https://en.wikipedia.org/wiki/Christofides_algorithm))
"""
...
md"""
The second step is to improve your solution iteratively through [local search](https://en.wikipedia.org/wiki/Local_search_(optimization)).
Basic principle:
...
- [``2``-opt](https://en.wikipedia.org/wiki/2-opt): remove $2$ edges from the tour and cross them
- ``k``-opt: remove $k$ edges from the tour and cross them
- [Lin-Kernighan](https://en.wikipedia.org/wiki/Lin%E2%80%93Kernighan_heuristic) heuristic, more precisely the [Helsgaun implementation](http://webhotel4.ruc.dk/~keld/research/LKH/)
"""
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "4af68e8f5ed5ed90a9e5c310b1bebcf56ed20ad8"
...
[[deps.WhereTraits]]
deps = ["Compat", "DataTypesBasic", "ExprParsers", "Graphs", "IterTools", "Markdown", "MetaGraphs", "Pipe", "ProxyInterfaces", "Reexport", "Setfield", "SimpleMatch", "StructEquality"]
git-tree-sha1 = "74b037c7d6da976611b087c4d611b4940d1528e3"
