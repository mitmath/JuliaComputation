...
Here is a map of the notation used in this notebook. The first seven rows belong to
the full three-dimensional Navier–Stokes problem. The later rows belong to the smaller
experiments below. In particular, **`A` is reused**: it means the initial amplitude in
the Burgers experiment and the wave amplitude in the reduced ODE.

| Symbol | Type | Count | Role in this notebook |
|:--|:--|--:|:--|
| $x \in \mathbb{R}^3$ (or $\mathbb{T}^3$) | independent variable | 3 | position in the full fluid |
| $t \ge 0$ | independent variable | 1 | time |
| $\mathbf{u}(x,t)$ | unknown field | 3 | fluid velocity; the main unknown |
| $p(x,t)$ | unknown field | 1 | pressure; enforces incompressibility |
| $\nu > 0$ | parameter | -- | kinematic viscosity |
| $\mathbf{f}(x,t)$ | given field | 3 | external body force per unit mass |
| $\mathbf{u}_0(x)$ | given field | 3 | initial velocity, $\mathbf{u}(x,0)=\mathbf{u}_0(x)$ |
| $\nabla$ | differential operator | -- | gradient; $\nabla p$ points in the direction of increasing pressure |
| $\nabla\!\cdot$ | differential operator | -- | divergence; $\nabla\!\cdot\mathbf{u}=0$ means incompressible |
| $\Delta$ | differential operator | -- | Laplacian; spreads out sharp features |
| $A$ | slider parameter | 1 | initial sine-wave amplitude in Burgers, then wave amplitude in the ODE |
| $u(x,t)$ | unknown field | 1 | one-dimensional velocity in Burgers; here `u` is scalar |
| $\nu$ | slider parameter | 1 | viscosity in the Burgers experiment |
| $\Theta(t),\ \Omega(t)$ | unknown functions | 2 | wave amplitudes in the reduced ODE |
| $\phi$ | slider parameter | 1 | angle controlling the ODE growth rate |
| $\lambda r$ | slider parameter | 1 | frequency scale in the reduced ODE |
| $Y(t)$ | unknown function | 1 | comparison-model amplitude in $\dot Y=cY^2$ |
| $c>0$ | slider parameter | 1 | self-amplification rate in the comparison model |
| $T_* = 1/(cY_0)$ | derived quantity | 1 | predicted finite blow-up time for the comparison ODE |
| $N$ | numerical parameter | 1 | number of grid points on the periodic interval |
| $\Delta x$ (`dx`) | numerical quantity | 1 | grid spacing |
| $\Delta t$ (`dt`) | numerical quantity | 1 | time step chosen for stability |

The word **unknown** means that the equations are asking us to find the function. The
word **data** or **parameter** means that it is supplied before solving. The symbols
$\nabla$, $\nabla\!\cdot$, and $\Delta$ are operations, not additional unknowns.
"""
